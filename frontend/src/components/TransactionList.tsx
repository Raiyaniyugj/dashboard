      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-100 shadow-xs">
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 focus:border-slate-300 text-xs rounded-xl focus:outline-hidden focus:ring-0 text-slate-700 transition"
          />
          {/* Suggestions Dropdown */}
          {isSearchFocused && searchSuggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-10 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              {searchSuggestions.map((suggestion, idx) => (
                <div
                  key={idx}
                  className="px-4 py-2.5 text-xs text-slate-600 hover:text-slate-900 hover:bg-slate-50 cursor-pointer flex items-center transition"
                  onClick={() => setSearch(suggestion)}
                >
                  <Search className="w-3.5 h-3.5 mr-2.5 text-slate-400" />
                  {suggestion}
                </div>
              ))}
            </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 p-1 rounded-xl">
            <span className="text-xs text-slate-500 pl-2 font-medium">From:</span>
            <input 
              type="date" 
              value={fromDate} 
              onChange={(e) => setFromDate(e.target.value)} 
              className="bg-transparent border-none text-xs font-mono text-slate-700 outline-none focus:ring-0 cursor-pointer p-1" 
            />
            ...
            {(fromDate || toDate) && (
              <button 
                ...
                className="px-2 py-1 mx-1 text-[10px] font-bold uppercase tracking-wider bg-rose-100 text-rose-600 rounded-lg hover:bg-rose-200 transition cursor-pointer"
              >
                Clear
              </button>
            )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-slate-900 border border-slate-200 bg-white rounded-lg cursor-pointer hover:bg-slate-50 transition print:hidden"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-slate-900 border border-slate-200 bg-white rounded-lg cursor-pointer hover:bg-slate-50 transition print:hidden"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-slate-400"><polyline points="6 9 6 2 18 2 18 9"></polyline><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><rect x="6" y="14" width="12" height="8"></rect></svg>
            <span>Print</span>
          </button>
        </div>
        ...
        <button
          onClick={onAddClick}
          className="inline-flex items-center justify-center gap-1.5 px-4 py-1.5 text-xs font-semibold text-white bg-slate-800 hover:bg-slate-900 rounded-lg cursor-pointer transition shadow-sm print:hidden"
        >
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-sans">
                {filteredAndSortedTransactions.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className={`py-3.5 px-4 text-right font-semibold text-sm ${t.type === 'income' ? 'text-emerald-600 font-bold' : 'text-rose-600'}`}>

  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style={{ backgroundColor: '#112D4E' }}>
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-40 -right-40 w-96 h-96 rounded-full blur-3xl opacity-10" style={{ backgroundColor: '#DBE2EF' }} />
      </div>
        ...
        <div className="backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border" style={{ backgroundColor: 'rgba(249,247,247,0.05)', borderColor: 'rgba(219,226,239,0.15)' }}>
          ...
          <div className="px-8 pt-10 pb-6 text-center" style={{ borderBottom: '1px solid rgba(219,226,239,0.12)' }}>
            <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-4" style={{ backgroundColor: 'rgba(63,114,175,0.2)', border: '1px solid rgba(63,114,175,0.4)' }}>
              <ShieldCheck className="w-7 h-7" style={{ color: '#DBE2EF' }} />
            </div>
            <h1 className="text-2xl font-bold tracking-tight" style={{ color: '#F9F7F7' }}>Set new password</h1>
            <p className="text-sm mt-1" style={{ color: '#DBE2EF' }}>Choose a strong, new password for your account</p>
          </div>

          <div className="px-8 py-7">
            {success ? (
              <div className="text-center space-y-4 py-4">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full" style={{ backgroundColor: 'rgba(63,114,175,0.2)', border: '1px solid rgba(63,114,175,0.4)' }}>
                  <CheckCircle className="w-8 h-8" style={{ color: '#DBE2EF' }} />
                </div>
                <div>
                  <h3 className="text-lg font-bold" style={{ color: '#F9F7F7' }}>Password reset!</h3>
                  <p className="text-sm mt-1" style={{ color: '#DBE2EF' }}>Redirecting you to login…</p>
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#DBE2EF' }}>New Password</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#3F72AF' }} />
                    <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                      placeholder="Min. 6 characters" required className="w-full pl-10 pr-11 py-3 rounded-xl text-sm outline-none transition"
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
                      placeholder="Re-enter your password" required className="w-full pl-10 pr-11 py-3 rounded-xl text-sm outline-none transition"
                      style={inputStyle} onFocus={focusIn} onBlur={focusOut} />
                ...
                >
                  {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><ShieldCheck className="w-4 h-4" /> Reset Password</>}
                </button>
              </form>
            )}
          </div>
